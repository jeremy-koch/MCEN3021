...
Pressure is higher upstream and lower downstream, and so the gradient points downstream. The negative of the gradient, $-\nabla p$, is what drives the flow. Then flow is occurring, but is being resisted by friction at the walls — the shear force at the walls points upstream. At steady-state, these forces balance each other.
:::
...
Nonzero, so the flow is rotational — consistent with the recirculating cell from part (a).
:::
